...
from lib.models.FERMT.gaussian import TokenGeneration
...
class BaseBackbone(nn.Module):
    def __init__(self):
        ...
        self.topk_win_Num = 3
        self.token_Generation = TokenGeneration(dim=192)
    ...
    def forward_features(self, z, x):        
        ...
        x_ext = self.token_Generation(z, x)
        # print(x_ext.shape) 

        x = combine_tokens(z, x, mode=self.cat_mode)
        x = torch.cat([x_ext,x], dim=1)
        x = self.pos_drop(x)
